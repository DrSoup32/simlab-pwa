...
export default function Control() {
  ...
  // Pearls
  // Pearl rows persisted in Control timeline (extend generic Pearl from socket with id/tag fields)
  type PearlRow = { id: string; text: string; tag?: string; at: number; by?: string; linkedAssetId?: string; hidden?: boolean; color?: string }
  ...
  // Orders
  useEffect(() => {
    // Incoming order events include extra metadata (timestamp, by, color, role)
    type IncomingOrder = Order & { at?: number; by?: string; learnerId?: string; color?: string; role?: string }
    const onOrder = (o: IncomingOrder) => {
      setOrders(prev => [o as any, ...prev])
      if (o.at) {
  ...
  // Pearls
  useEffect(() => {
    // Assume server sends { id,text,tag,at,by,color?,linkedAssetId?,hidden? }
    const onPearl = (p: any) => {
      const row: PearlRow = {
        id: p.id || `pearl-${Date.now()}`,
        text: p.text || '',
        tag: p.tag,
        at: p.at || Date.now(),
        by: p.by,
      ...
      setPearls(prev => prev.some(x => x.id === row.id) ? prev : [row, ...prev])
      setHistory(prev => [{ when: row.at, kind: 'pearl', title: row.tag ? `Pearl (${row.tag})` : 'Pearl' }, ...prev])
    }
    const onPearlEdit = (p: any) => {
      setPearls(prev => prev.map(x => x.id === p.id ? { ...x, text: p.text, tag: p.tag, hidden: p.hidden } : x))
    }
              ...
              <div key={`${item.asset.id}-${item.at}-${i}`} style={{ border:'1px solid #eee', borderRadius:8, padding:8, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <div style={{ display:'flex', gap:6 }}>
                  <button
                    ...
                    style={{ padding:'6px 10px', border:'1px solid #0aa', borderRadius:8 }}
                    title="Focus on Stage"
                  >
                    Jump
                  </button>
                  {item.asset.contentUrl && (
                    <a
                      className="toolbtn"
                      href={item.asset.contentUrl}
                      target="_blank"
                      rel="noreferrer"
                      style={{ padding:'6px 10px', border:'1px solid #ccc', borderRadius:8 }}
                    >
          ...
          <div style={{ display:'flex', gap:6, marginBottom:6 }}>
            ...
            <button
              onClick={() => {
                ...
                if (target === 'STAGE') {
                  socket.emit('control:stage:broadcast', { caseId, text, at: Date.now() })
                } else if (target === 'ALL') {
                  socket.emit('control:message', { caseId, text, at: Date.now() })
                } else {
                  socket.emit('control:message', { caseId, text, at: Date.now(), targetId: target })
                }
  ...
  {/* search removed per request */}
      ...
      <Section 
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {orders.map((raw, i) => {
            const o = raw as Order & { at?: number; by?: string; color?: string; role?: string }
            return (
            <div key={i} style={{ border:'1px solid #ddd', borderRadius:8, padding:10, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  ...
                  {o.color && <span style={{ display:'inline-block', width:10, height:10, borderRadius:'50%', background: o.color }} />}
                  {o.by && <span>from {o.by}</span>}
                  {o.role && <span style={{ fontSize:12, opacity:.8, border:'1px solid #ddd', borderRadius:8, padding:'2px 6px' }}>{o.role}</span>}
                </div>
                {o.at && <div style={{ fontSize:12, opacity:.7 }}>{new Date(o.at).toLocaleTimeString()}</div>}
              </div>
              <div style={{ display:'flex', gap:6 }}>
