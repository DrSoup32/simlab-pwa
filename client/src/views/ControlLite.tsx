...
export default function ControlLite() {
  ...
  const [learners, setLearners] = useState<Array<{ id:string; name:string; color?:string }>>([]);

  useEffect(() => { listCases().then(setCases).catch(()=>{}); }, []);
  useEffect(() => {
    if (!caseId) return;
    getSession(caseId).then(s => setItems(s.stage || [])).catch(()=>{});
    // keep Stage list live
  ...
  useEffect(() => {
    ...
    const onList  = (p:{caseId?:string; learners:Array<{id:string;name:string;color?:string}>}) => {
  ...
  const jump = (it: StageItem) => {
    socket.emit("control:stage:select", { caseId, assetId: it.asset.id, at: it.at });
  };
