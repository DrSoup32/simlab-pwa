// server/src/monitorRoutes.js  (ESM)
...
/** Register /monitor.jpg and /api/monitor/send */
export function registerMonitorRoutes(app, io, opts = {}) {
    // optional overrides:
  // snapshotPath: "C:\\monitor-feed\\monitor.jpg",
  // uploadBase: "http://localhost:4000",
  ...
  // 1) Serve the current snapshot (no caching)
  ...
  // 2) Send current snapshot to Stage
  app.post("/api/monitor/send", async (req, res) => {
    try {
      const caseId =
        req.query.caseId ||
        req.body?.caseId ||
        env("DEFAULT_CASE_ID", "case-anemia");
      ...
      const info = await r.json(); // { url, originalName, mime }

      const asset = {
        id: `monitor-${Date.now()}`,
        title: `Monitor ${new Date().toLocaleTimeString()}`,
        type: classify(info.mime),
        contentUrl: info.url,
      };
